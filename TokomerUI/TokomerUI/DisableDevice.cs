...
ref PropertyChangeParameters classInstallParams, int classInstallParamsSize);
    ...
    public sealed class DeviceHelper
    {
        ...
        static public string[] QueryDosDevice()
        {
            ...
            while (returnSize == 0)
            {
                ...
                if (mem != IntPtr.Zero)
                {
                    ...
                    try
                    {
                        returnSize = NativeMethods.QueryDosDevice((string)null, mem, (uint)maxSize);
                        if (returnSize != 0)
                        {
                        ...
                        else if (System.Runtime.InteropServices.Marshal.GetLastWin32Error() == NativeMethods.ERROR_INSUFFICIENT_BUFFER)
                        //maybe better
                        ...
                        {
                            Marshal.ThrowExceptionForHR((int)NativeMethods.GetLastError());
                        }
                    }
                    finally
                    {
        ...
        static public string GetRealPath(string path)
        {
            ...
            NativeMethods.QueryDosDevice(driveLetter, pathInformation, 250);
